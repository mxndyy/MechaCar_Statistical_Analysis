...
#3 Create a total_summary  df using the summarize() function to get the mean,  median, variance and std dev of the suspension coil's PSI  column
total_summary <- mecha_coil  %>% summarize(Mean=mean(PSI),
                                           Median=median(PSI),
                                           Variance=var(PSI),
                                           SD=sd(PSI),.groups = 'keep')

#4 Create a lot_summary df using the group_by() and the summarize() functions to group each mfg lot.
...
plt2 <- ggplot(mecha_coil,aes(x=Manufacturing_Lot,y=PSI)) #import into ggplot2
plt2 + geom_boxplot()

### DELIVERABLE 3

#1 use t.test() to determine if the PSI across all lots is statistically diff from the population mean of 1,500 PSI
